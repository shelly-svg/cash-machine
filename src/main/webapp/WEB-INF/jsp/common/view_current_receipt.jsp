...
<c:set var="title" value="view.receipt.page.title"/>
...
<div id="container">
    ...
    <div id="add_product_form">
        ...
        <fmt:formatDate value="${sessionScope.currentReceipt.createTime}" type="date" pattern="EEE, d MMM yyyy HH:mm:ss"/>
        <hr>
        ...
        <hr>
        <c:if test="${empty requestScope.currentReceiptProductMap}}"><h6><fmt:message
                key="view_current_receipt_jsp.has.no.products"/></h6></c:if>
        ...
        <c:if test="${sessionScope.lang == 'ru'}">
        <h5>
                <c:out value="${product.key.nameRu}"/>
            </c:if>
            <c:if test="${sessionScope.lang == 'en'}">
            <h5><c:out value="${product.key.nameEn}"/>
                </c:if>
                ; <fmt:message key="view_current_receipt_jsp.products.amount"/> = <c:out
                        value="${product.value.toString()}"/></h5>
            </c:forEach>
            </c:if>
            <hr>
            ...
            <form action="controller" method="post">
                ...
                <button type="submit" class="close_receipt_btn" name="submit"><fmt:message
                        key="view_current_receipt_jsp.close.receipt.btn"/></button>
            </form>
            </c:if>
            <c:if test="${sessionScope.currentReceipt.receiptStatus.name().toLowerCase() == 'new_receipt'
            ...
            <form action="controller" method="post">
                ...
                <button type="submit" class="close_receipt_btn" name="submit"><fmt:message
                        key="view_current_receipt_jsp.cancel.receipt.btn"/></button>
            </form>
            </c:if>
            </c:if>
    </div>
